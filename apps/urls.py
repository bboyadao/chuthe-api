from django.urls import re_path

from rest_framework import routers
from apps.views import Application
from oauth2_provider.views import AuthorizationView, TokenView, RevokeTokenView, IntrospectTokenView, \
    AuthorizedTokenDeleteView, AuthorizedTokensListView, JwksInfoView, UserInfoView, ConnectDiscoveryInfoView
app_name = "apps"


apps_routes = routers.DefaultRouter()
apps_routes.register('application', Application, basename='applications')


base_urlpatterns = [
    re_path(r"^authorize/$", AuthorizationView.as_view(), name="authorize"),
    re_path(r"^token/$", TokenView.as_view(), name="token"),
    re_path(r"^revoke_token/$", RevokeTokenView.as_view(), name="revoke-token"),
    re_path(r"^introspect/$", IntrospectTokenView.as_view(), name="introspect"),
]

#
management_urlpatterns = [
    # Application management views Done
    # re_path(r"^applications/$", ApplicationList.as_view(), name="list"),
    # re_path(r"^applications/register/$", ApplicationRegistration.as_view(), name="register"),
    # re_path(r"^applications/(?P<pk>[\w-]+)/$", ApplicationDetail.as_view(), name="detail"),
    # re_path(r"^applications/(?P<pk>[\w-]+)/delete/$", ApplicationDelete.as_view(), name="delete"),
    # re_path(r"^applications/(?P<pk>[\w-]+)/update/$", ApplicationUpdate.as_view(), name="update"),


    # Token management views
    re_path(r"^authorized_tokens/$", AuthorizedTokensListView.as_view(), name="authorized-token-list"),
    re_path(
        r"^authorized_tokens/(?P<pk>[\w-]+)/delete/$",
        AuthorizedTokenDeleteView.as_view(),
        name="authorized-token-delete",
    ),
]

oidc_urlpatterns = [
    re_path(
        r"^\.well-known/openid-configuration/$",
        ConnectDiscoveryInfoView.as_view(),
        name="oidc-connect-discovery-info",
    ),
    re_path(r"^\.well-known/jwks.json$", JwksInfoView.as_view(), name="jwks-info"),
    re_path(r"^userinfo/$", UserInfoView.as_view(), name="user-info"),
]


urlpatterns = apps_routes.urls + base_urlpatterns + management_urlpatterns + oidc_urlpatterns
